...
<main class="shell">
  ...
  <section class="grid gtdpage">
    <article class="panel compact">
      <h2>GTD</h2>
      <div class="lanes">
        {#each activeStatuses as status}
          <div class="lane"><h3>{status}<b>{byStatus(status).length}</b></h3>{#each byStatus(status).slice(0, 7) as item}<article class="card"><strong>{item.title}</strong><code>{item.path}</code></article>{/each}</div>
        {/each}
      </div>
    </article>
  </section>
...
<style>
  ...
  i{display:inline-block;width:9px;height:9px;border-radius:50%;background:#607080;box-shadow:0 0 7px #607080}.ok{background:#28ff9c;box-shadow:0 0 10px #28ff9c}.warn{background:#ffd447;box-shadow:0 0 10px #ffd447}.down{background:#ff3f6e;box-shadow:0 0 10px #ff3f6e}.grid{display:grid;gap:12px;margin-top:12px}.gtdpage{grid-template-columns:1fr}.compact{padding:14px}h2{font-size:15px;margin:0 0 12px;text-transform:uppercase;letter-spacing:.08em;color:#9beafe}.card code{color:#75d9ff;font-size:11px;word-break:break-word}.lanes{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px}.lane{padding:10px;border-radius:12px;background:rgba(0,0,0,.24)}.lane h3{margin:0 0 8px;color:#67e8f9;font-size:12px;text-transform:uppercase;letter-spacing:.08em}.lane h3 b{float:right;color:#ff5d85}.card{padding:9px;margin-bottom:8px;border-radius:10px;background:rgba(16,31,48,.75);border:1px solid rgba(110,231,255,.1)}.card strong{display:block;color:#fff;font-size:13px;margin-bottom:4px}
  @media(max-width:1000px){.topbar{height:auto;min-height:42px;flex-wrap:wrap;padding:8px 12px}.ledline{order:3;flex-basis:100%;flex-wrap:wrap}}
</style>
