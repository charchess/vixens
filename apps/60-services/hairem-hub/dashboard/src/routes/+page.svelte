...
<main class="shell">
  ...
  <section class="grid main2">
    <article class="panel statuspanel">
      <h2>LLM</h2>
      <div class="llmgrid">
        {#each data.runtime.llms as llm}
          <div class="llmbox">
            <header><span><i class:ok={llm.led === 'ok'} class:warn={llm.led === 'warn'} class:down={llm.led === 'down'}></i>{llm.id}</span><code>{llm.detail}</code></header>
            {#if llm.loaded.length === 0}<p class="muted">Aucun modèle chargé.</p>{/if}
            {#each llm.loaded as m}<div class="row hot"><span>{m.name}</span><code>loaded</code></div>{/each}
            {#each llm.models as m}<div class="row"><span>{m.name}</span><code>{m.size || ''} {m.quant || ''}</code></div>{/each}
          </div>
        {/each}
      </div>
    </article>
...
<style>
  ...
  i{display:inline-block;width:9px;height:9px;border-radius:50%;background:#607080;box-shadow:0 0 7px #607080}.ok{background:#28ff9c;box-shadow:0 0 10px #28ff9c}.warn{background:#ffd447;box-shadow:0 0 10px #ffd447}.down{background:#ff3f6e;box-shadow:0 0 10px #ff3f6e}.grid{display:grid;gap:12px;margin-top:12px}.metrics{grid-template-columns:repeat(4,minmax(0,1fr))}.metric{padding:12px}.metric span,.muted,small{color:#8aaec0}.metric strong{display:block;color:#fff;font-size:28px;line-height:1.1;margin:5px 0}.main2{grid-template-columns:1.15fr .85fr}.two{grid-template-columns:1.4fr .6fr}.statuspanel,.compact{padding:14px}h2{font-size:15px;margin:0 0 12px;text-transform:uppercase;letter-spacing:.08em;color:#9beafe}.docs{display:grid;gap:7px}.llmgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:10px}.llmbox{padding:10px;background:rgba(0,0,0,.22);border-radius:12px}.llmbox header,.row{display:flex;justify-content:space-between;gap:12px;align-items:center}.llmbox header{margin-bottom:8px}.llmbox header span,.row span{display:flex;gap:8px;align-items:center}.row{padding:8px 10px;background:rgba(0,0,0,.22);border-radius:10px;margin-top:7px}.row code,.card code,.doc code,.bank code,.llmbox code{color:#75d9ff;font-size:11px;word-break:break-word}.hot{border:1px solid rgba(40,255,156,.22)}.banks{display:grid;gap:9px}.bank{padding:10px;background:rgba(0,0,0,.22);border-radius:12px}.bank header{display:flex;justify-content:space-between;gap:10px;margin-bottom:8px}.bankgrid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.bankgrid span{color:#9fb9c8}.bankgrid b{color:#fff}.lanes{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:10px}.lane{padding:10px;border-radius:12px;background:rgba(0,0,0,.24)}.lane h3{margin:0 0 8px;color:#67e8f9;font-size:12px;text-transform:uppercase;letter-spacing:.08em}.lane h3 b{float:right;color:#ff5d85}.card,.doc{padding:9px;margin-bottom:8px;border-radius:10px;background:rgba(16,31,48,.75);border:1px solid rgba(110,231,255,.1)}.card strong,.doc strong{display:block;color:#fff;font-size:13px;margin-bottom:4px}
  @media(max-width:1000px){.metrics,.main2,.two{grid-template-columns:1fr}.topbar{height:auto;min-height:42px;flex-wrap:wrap;padding:8px 12px}.ledline{order:3;flex-basis:100%;flex-wrap:wrap}}
</style>
