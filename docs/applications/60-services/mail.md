...
Docker Mailserver v16 moves Dovecot from 2.3 to 2.4. Dovecot 2.4 removes the legacy one-character setting variables: the bounded `userPrincipalName` filter must use `(&(objectClass=user)(userPrincipalName=%{user}))` for both userdb and passdb. Do not use the former `%u`, which resolves no user under Dovecot 2.4. The Dovecot userdb mapping must also use DMS v16 syntax: `=home=/var/mail/%d/%n/,=mail_path=/var/mail/%d/%n/,=uid=5000,=gid=5000`. Do not retain the legacy double-prefix (`==home`) or `maildir:` mapping: DMS v16 rejects the former and maps `mail_path` directly.

The DMS `user-patches.sh` hook creates `tmp` and `db` with Amavis ownership (`998:998` for the DMS v16 Debian 13 image) and restrictive modes after DMS setup and before Amavis starts. This is required for the localhost content-filter listener on port `10024`; it contains no mailbox data and is deliberately separate from the retained Maildir and mail-state PVCs. Any future image change that changes the Amavis runtime UID/GID must update this hook and its StatefulSet rollout annotation together.
