...
Docker Mailserver v16 moves Dovecot from 2.3 to 2.4. The LDAP service-account bind and the bounded `userPrincipalName` filter remain unchanged, but the Dovecot userdb mapping must use DMS v16 syntax: `=home=/var/mail/%d/%n/,=mail_path=/var/mail/%d/%n/,=uid=5000,=gid=5000`. Do not retain the legacy double-prefix (`==home`) or `maildir:` mapping: DMS v16 rejects the former and maps `mail_path` directly.
